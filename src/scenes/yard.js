...
class YardState extends MulleState {
  ...
  create () {
    ...
    // 04.DXR member 145 = 04b001v0 (yard with garage/mailbox)
    background.setDirectorMember('04.DXR', 145)
    ...
    if (this.game.mulle.user.toYardThroughDoor) {
      ...
      this.door_garage = new MulleButton(this.game, 320, 240, {
        ...
        imageHover: ['04.DXR', 41],
        click: (a) => {
          // 00b016v0: entering the garage from the yard (main door)
          this.game.mulle.activeCutscene = '00b016v0'
          this.game.state.start('garage')
        }
      })
  ...
  kickTelephone (mission) {
    ...
    // Create small phone sprite (03b003v0 = member 102) - fallback to graphics if sprite not available
    this.phoneSmall = new MulleSprite(this.game, 50, 50)
    if (!this.phoneSmall.setDirectorMember('03.DXR', 102)) {
      // Fallback: create simple phone graphic
      console.warn('[Yard] TeleSmall sprite not found, using fallback')
      const phoneGraphic = this.game.add.graphics(50, 50)
      phoneGraphic.beginFill(0x333333)
      phoneGraphic.drawRoundedRect(-10, -15, 20, 30, 3)
      phoneGraphic.endFill()
      this.phoneSmall = phoneGraphic
    } else {
    ...
    // Play ringtone (03e001v0) - may not exist
    this.currentSound = this.game.mulle.playAudio('03e001v0')
    
    this.frameCount = 0
    this.currentMission = mission
    
    // If no ringtone audio, skip to finishPhone after short delay
    if (!this.currentSound) {
      console.warn('[Yard] Ringtone 03e001v0 not found, skipping ring animation')
  ...
  finishPhone () {
    ...
    // Show big phone overlay (03b002v0 = member 100) - fallback if not available
    this.phoneBig = new MulleSprite(this.game, 320, 240)
    if (!this.phoneBig.setDirectorMember('03.DXR', 100)) {
      // Fallback: create phone overlay graphic
      console.warn('[Yard] Big phone sprite not found, using fallback')
      const phoneBg = this.game.add.graphics(320, 240)
      phoneBg.beginFill(0x222222, 0.8)
      phoneBg.drawRoundedRect(-100, -150, 200, 300, 10)
      phoneBg.endFill()
      phoneBg.beginFill(0x444444)
      phoneBg.drawCircle(0, -50, 80)
      phoneBg.endFill()
      this.phoneBig = phoneBg
    } else {
    ...
    if (this.currentMission && this.currentMission.sound) {
      ...
      // No audio, hang up after short delay
  ...
  hangUp () {
    ...
    // Also mark as completed since telephone missions are complete after listening
    this.missionSystem.markAsCompleted(this.currentMission.id)
  ...
  finishMail () {
    ...
    // Mark current mail mission as completed
