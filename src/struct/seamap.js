...
class MulleSeaMap {
  constructor (game, id) {
    ...
    // underMapImage layer (#148) - background beneath main map
    this.underMapImage = null
    
    // Map special properties (#149) - #windspeed, #Fog
    this.Special = null
  ...
  reset () {
    ...
    // underMapImage layer (#148)
    this.underMapImage = this._data.underMapImage
    
    // Map special properties (#149)
    this.Special = this._data.Special
  ...
  /**
   * Get special property value
   * Map special properties (#149) - #windspeed, #Fog
   * @param {symbol} argProp - Property name (e.g., #windspeed, #Fog)
   * @returns {*} Property value or VOID
   */
  getSpecial (argProp) {
    if (!argProp) {
      return this.Special
    ...
    if (this.Special && typeof this.Special === 'object') {
      return this.Special[argProp]
    }
    return undefined
  }
  
  /**
   * Get underMapImage layer (#148)
   * @returns {string|null} underMapImage name
   */
  getUnderMapImage () {
  ...
  toJSON () {
    return {
      ...
      underMapImage: this.underMapImage,
