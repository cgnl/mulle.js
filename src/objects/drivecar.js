...
class MulleDriveCar extends MulleSprite {
  constructor (game) {
    ...
    var directions = { '-2': 78, '-1': 94, '0': 110, '1': 126, '2': 142 }
    ...
    */
    ...
    // Lingo MovieScript 9 uses decimalPrec = 100 for DirectionList
    ...
    for (var i = 1; i <= this.nrOfDirections; i++) {
      ...
      var xPart = (Math.sin(tmp) * this.decimalPrec)
      var yPart = -(Math.cos(tmp) * this.decimalPrec)
      this.directionList.push(new Phaser.Point(xPart, yPart))
    }

    // Unit vectors derived from DirectionList (scaled by decimalPrec)
    this.directionUnitList = []
    for (var i = 1; i <= this.directionList.length; i++) {
      var raw = this.directionList[i - 1]
      this.directionUnitList.push(new Phaser.Point(
        raw.x / this.decimalPrec,
        raw.y / this.decimalPrec
      ))
    }

    this.wheelPositions = []
    for (var i = 1; i <= this.directionUnitList.length; i++) {
      var x = 8 * this.directionUnitList[ i - 1 ].x
      var y = 8 * this.directionUnitList[ i - 1 ].y
  ...
  correctDirection (i) {
    var dir = i % this.nrOfDirections
    if (dir <= 0) return dir + this.nrOfDirections
    return dir
  }
  ...
  drive () {
    ...
    if (this.speed) {
      ...
      for (var d = 0; d <= 1; d++) {
        ...
        var ang = this.directionUnitList[ this.correctDirection(this.direction + dir) - 1 ].clone()
      ...
      var currentAngle = this.directionUnitList[ this.direction - 1 ]
    ...
    if (this.acceleration) {
      if (this.memeMode) {
        ...
        if (this.stopTimer === 0) {
          ...
          // var ang = this.directionUnitList[ this.direction - 1 ].clone();
