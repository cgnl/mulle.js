...
class MulleSubtitle {
  ...
  makeObject () {
    ...
    this.textObject = new Phaser.Text(this.game, 0, 0, '', {
      ...
      // BUG FIX #20: Subtitle missing multi-line text handling verification
      wordWrap: true,
      wordWrapWidth: 620
    })

    // BUG FIX #10: Subtitle text bounds incorrect position (hardcoded y=420)
    // Original should position relative to screen height for proper multi-line support
    this.textObject.setTextBounds(0, 400, 640, 80)
  ...
  showLine (text, actor, audioDuration) {
    if (!text || text === '-') {
      // Skip empty lines and '-' placeholder (Lingo silent marker)
      return false
    }
    ...
    // BUG FIX #9: Subtitle Auto-Dismiss Timing Formula Incorrect
    // Original uses audio-driven timing, not logarithmic formula
    // If audioDuration provided, use that; otherwise use reasonable estimate
    var del
    if (audioDuration) {
      del = audioDuration
    } else {
      // Fallback: use reading speed estimate (250 words/min = ~4 chars/sec)
      del = Math.max(2000, text.length * 50) // Min 2s, then 50ms per character
    }
