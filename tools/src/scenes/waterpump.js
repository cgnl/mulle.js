 ...
 * 
 * MISSION LOGIC (from Lingo ParentScript):
 * - Mission 7: Related to Diary - give #Diary item when mission 7 is given
 * - Mission 8: Related to Suit - give #Suit item when mission 7 complete AND mission 8 given
 * 
 * Flow:
 * 1. If mission 7 completed AND mission 8 given → give Suit, complete mission 8, go to "suit" marker
 * 2. Else if mission 7 given AND no Diary → give Diary, go to "Diary" marker
 * 3. Else → "Leave" marker (nothing to do)
 */
...
class WaterPumpState extends MulleState {
  ...
  create () {
    ...
    // Draw the player's boat at position (320, 155)
    this.boat = new MulleBuildBoat(this.game, 320, 155, null, true, false)
    ...
    // === MISSION LOGIC ===
    // Determine what marker/action to take based on mission state
    // Original Lingo flow from startMovie:
    // - go("StartDiary") if myMarker = "Diary"
    // - go("Start") if myMarker = "Leave"
    // - go("suit") if myMarker = "suit"
    this.myMarker = this.determineMissionMarker()

    console.log('[WaterPump] Scene created, marker:', this.myMarker)

    // === FRAME FLOW ===
    // Execute marker flow immediately (matching original Lingo startMovie behavior)
    this.handleMissionMarker()
  }

  /**
   * Determine the mission marker based on current mission state
   ...
   *   end if
   * 
   * @returns {string} Marker name: 'suit', 'Diary', 'leave', or 'Leave'
   */
  determineMissionMarker () {
    const user = this.game.mulle.user

    // Check if mission 7 is completed
    if (user.isMissionCompleted(7) > 0) {
      // Mission 7 complete - check for mission 8 (Suit)
      if (user.isMissionGiven(8)) {
        // BUG FIX #1.1: Mission completion moved to AFTER dialogue in playSuitSequence()
        // Don't give suit or complete mission here - do it after player sees dialogue
        console.log('[WaterPump] Mission 8 ready - will complete AFTER suit sequence')
        return 'suit'
      } else {
        // Mission 7 complete but mission 8 not given yet
        // Original Lingo: set myMarker to "leave" (lowercase)
        return 'leave'
      }
    } else {
      // Mission 7 not completed - check if it's given
      if (user.isMissionGiven(7)) {
        // Mission 7 given - check for Diary
        if (!user.isInInventory('#Diary')) {
          // Give the diary
          user.setInInventory('#Diary')
          console.log('[WaterPump] Diary given for mission 7')
          return 'Diary'
        } else {
          // Already have diary
          // Original Lingo: set myMarker to "Leave" (capitalized)
          return 'Leave'
        }
      } else {
        // Mission 7 not given
  ...
  /**
   * Handle the mission marker - play appropriate dialogue/scene
   * Based on original Lingo go() commands:
   *   if myMarker = "Diary" then go("StartDiary")
   *   if myMarker = "Leave" then go("Start")
   *   if myMarker = "suit" then go("suit")
   * 
   * BehaviorScript 5 (exitFrame): go(the myMarker of gDir)
   * BehaviorScript 6 & 7 (exitFrame): go("leave") - redirects to leave marker
   * 
   * NOTE: The switch statement provides a functional equivalent to the original Lingo go()
   * commands for frame routing. Each case maps to a specific frame label from the Director
   * movie and executes the corresponding sequence (suit, Diary, leave, etc).
   */
  handleMissionMarker () {
    switch (this.myMarker) {
      case 'suit':
        // go("suit") - Play suit found sequence
        this.playSuitSequence()
        break
      case 'Diary':
        // go("StartDiary") - Play diary found sequence
        this.playDiarySequence()
        break
      case 'leave':
      case 'Leave':
      default:
        // go("Start") for Leave, or go("leave") for leave
        // Both lead to the default intro/leave flow
   ...
   /**
    * Play the suit found sequence (Mission 8 reward)
    */
   playSuitSequence () {
    console.log('[WaterPump] Playing suit sequence')
    // Play suit-related dialogue
    // TODO: Identify correct dialogue ID for suit reward
    this.playDialogue('85d003v0', () => {
      // BUG FIX #1.1: Complete mission AFTER dialogue, not before
      const user = this.game.mulle.user
      user.setInInventory('#Suit')
      user.addCompletedMission(8)
      console.log('[WaterPump] Mission 8 completed AFTER dialogue - Suit given')
      this.showMessage('Je hebt een duikpak gevonden!')
    })
  }

  /**
   * Play the diary found sequence (Mission 7 item)
   */
  playDiarySequence () {
    console.log('[WaterPump] Playing diary sequence')
    // Play diary-related dialogue
    // TODO: Identify correct dialogue ID for diary
    this.playDialogue('85d002v0', () => {
      this.showMessage('Je hebt een dagboek gevonden!')
    })
  }
