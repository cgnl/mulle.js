...
class WorldSelectState extends MulleState {
  ...
  create () {
    ...
    this.createWorldOption({
      ...
      description: scheepswerfUnlocked 
        ? 'Bouw boten bij Mulle aan de kade!'
  ...
  showLockedMessage () {
    ...
    // STUB: asset not found in ISO — 02e001v0 (locked-world sound)
  }
  ...
  selectWorld (worldId) {
    ...
    if (worldId === 'blauwwater') {
      ...
      // Go to boat yard (new!)
      // Check if user has seen the intro movie
      if (!this.game.mulle.user.seenZeeIntro) {
        // Play the full intro movie (Miel builds vlot/kano/boot, arrives at quay)
        this.game.state.start('zee_intro')
      } else {
        // User has seen intro, go directly to boatyard
